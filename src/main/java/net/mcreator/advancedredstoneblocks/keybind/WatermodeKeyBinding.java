...
package net.mcreator.advancedredstoneblocks.keybind;
...
import net.mcreator.advancedredstoneblocks.procedures.FloatswaOnEntityTickUpdateOnKeyPressedProcedure;
...
public class WatermodeKeyBinding extends AdvancedredstoneblocksModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public WatermodeKeyBinding(AdvancedredstoneblocksModElements instance) {
		super(instance, 116);
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					AdvancedredstoneblocksMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				FloatswaOnEntityTickUpdateOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
